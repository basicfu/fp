...
// 但把 *store.SessionStore、*store.RevokePublisher 与 *store.EpochStore 本身
// 也交给调用方——有些测试需要绕过 SessionService 直接摆弄 Redis 状态
// （比如抢先占住一把锁），或者直接订阅撤销事件，光拿到 *service.SessionService
// 做不到这一点。
func newSessionParts(t *testing.T) (*store.SessionStore, *store.RevokePublisher, *store.EpochStore, *fakeClock) {
	t.Helper()
	rdb := testsupport.NewTestRedis(t)
	return store.NewSessionStore(rdb),
		store.NewRevokePublisher(rdb),
		store.NewEpochStore(rdb),
...
func newSessionService(t *testing.T) (*service.SessionService, *fakeClock) {
	...
	return service.NewSessionServiceWithClock(st, pub, ep, clk.Now), clk
}
