...
**fp 不依赖 fp-im。** fp 自带一条极简 SSE 端点用于控制面推送（配置变更 / 策略变更 / 撤销通知），不复用 fp-im，避免 fp↔fp-im 循环依赖，保证 fp 能独立部署运行。

### 3.3 四条通信链路

| 链路 | 协议 | 理由 |
|---|---|---|
| SDK ↔ fp | **HTTP/JSON + SSE** | 低频管理调用；UI 与 SDK 共用一套 API；可 curl 调试；过 nginx 无特殊配置 |
...
- 高频路径（token 校验、鉴权判定）按原则 2 全在本地，不走网络。剩下的是低频管理调用 + 单向推送，gRPC 的性能优势兑现不了。
- 推送是单向的（fp→SDK），SSE 足够，用不上双向流。
- 管理 UI 是 SPA，浏览器调不了 gRPC（需 grpc-web + 代理）。fp 无论如何都要有 HTTP API，再加一套 gRPC 是双倍维护成本。
- fp 是全局单点，排障时能 `curl` 比什么都重要。
- 明确要求"不再使用过于复杂的库"，protoc / buf / grpc-gateway 全套工具链与此相反。

**强类型契约的替代方案**：写 OpenAPI spec，用 `oapi-codegen` 生成 Go client 和 server 接口。获得类型安全、代码生成、多语言客户端，不引入 gRPC 的运维复杂度。
...
3s 现在每请求查一次 Redis，但那是**同进程直连**（`redis.GetString(TokenPrefix + tid)`，<1ms）。fp 化之后业务方要校验 token 就得**跨服务调 fp**，变成一次 HTTP 调用（3–10ms），且 fp 会成为每个请求路径上的强依赖——**fp 一挂，所有接入方立刻全挂**。本地缓存同时解决延迟和故障域两个问题。

**方案对比：**

| | 3s 现状（每次查 Redis） | 纯 JWT + 撤销名单 | **opaque + 缓存 + 推送** |
|---|---|---|---|
| 每请求网络 | 1 次（跨服务后 3–10ms） | 0 | ~0（命中率 >99%） |
| 撤销延迟 | 立即 | 推送延迟 | 推送延迟 |
...
```
SDK 本地缓存 cache_ttl=30s
  → 回源 fp 的读 QPS ≈ 100,000 / 30 ≈ 3,300
如果每次回源都写延期
  → 3,300 QPS 的写
```

3,300 写 QPS 对 Redis 无所谓，但 session 落 PostgreSQL 时代价完全不同：每次 `UPDATE` 产生 WAL、产生 dead tuple 触发 autovacuum、同一行并发 `UPDATE` 造成行锁排队。而这些写业务上毫无价值（把过期时间从 `t+3600` 改成 `t+3630`）。且 fp 是所有接入方共享的单点，写压力是全部业务方的总和。
...
- **fp 管理员账号与元权限**——与业务用户完全隔离的一套，不与业务权限混用
...
|---|---|---|
| 语言 | Go | |
| HTTP | `net/http` + `chi` | Go 1.22+ 的 ServeMux 已支持 `GET /users/{id}`，chi 只补中间件链和路由组，约 1500 行，完全兼容标准库 |
...
| API 契约 | OpenAPI + `oapi-codegen` | 类型安全 + 代码生成，不引入 gRPC 工具链 |
...
| SDK↔fp 用 HTTP 不用 gRPC | 3.3 | 高频路径全在本地，gRPC 优势兑现不了；UI 也要用这套 API |
